...
public class MedicinesRVAdapter extends RecyclerView.Adapter {
    private ArrayList<MedicinesVo> mALMedicines;
    private LayoutInflater mInflate;
    private Context mContext;
    private int mCount;

    public MedicinesRVAdapter(Context context, ArrayList<MedicinesVo> medicines) {
        this.mContext = context;
        this.mALMedicines = medicines;
        this.mContext = context;
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        myViewHolder.tvItem.setText(mALMedicines.get(position).getItem());
        myViewHolder.tvManufacturer.setText(mALMedicines.get(position).getManufacturer());
        myViewHolder.llAdd.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
//                countAmountDialog(position, mALMedicines.get(position).getItem(), mALMedicines.get(position).getManufacturer());
                addBookmarkDialog(position, mALMedicines.get(position).getItem(), mALMedicines.get(position).getManufacturer());
            }
        });
    }
                ...
                .setNegativeButton("추가",
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                MainActivity.sALMedicinesBookmark.add(new BasketVo(item, manufacturer, 1));
                                FirebaseBasketDB.getInstance(mContext).writeDB("의약품 북마크", new BasketVo(item, manufacturer, 1));
                            }
                        })
                .setCancelable(false)
                .show();
...
//                                        if (MainActivity.sALMedicinesBasket.get(i).getItem().equals(item))
//                                            MainActivity.sALMedicinesBasket.remove(i);
//                                    }
//
//                                    MainActivity.sALMedicinesBasket.add(new BasketVo(item, manufacturer, Integer.parseInt(etInputCount.getText().toString())));
////                                    Toast.makeText(mContext, "바구니에 추가되었습니다.", Toast.LENGTH_SHORT).show();
//                                    FirebaseBasketDB.getInstance(mContext).writeDB("의약품", new BasketVo(item, manufacturer, Integer.parseInt(etInputCount.getText().toString())));
//                                }
//                            }
//                        })
//                .setCancelable(false)
